...
MECTAGGER = None # MeCab.Tagger("")

USE_CUDA = torch.cuda.is_available()
device = torch.device("cuda" if USE_CUDA else "cpu")

NW_FILE = '../AllAbout.nwords.csv'
DATA_FILE = '../AllAboutTiny.idx2.txt'

def tagging(str_in, retWithList=False):
    res = res #MECTAGGER.parseToNode(str_in)
    ret = []
    while res:
...
def taggingFast(string_in, retWithList=False):
    ret = []
    # txt = MECTAGGER.parse(string_in.encode('utf-8', 'ignore'))
    txt = MECTAGGER.parse(string_in)
    lines = txt.split('\n')
    for li in lines:
        suf = li.split('\t')[0]
        ret.append(suf)
...
def read_w2idx_idx2w(nw_file='', min_freq=10):
    w2idx = {}
    idx2w = {PAD_token:'PAD', UNK_token:'[?]', SOS_token:'SOS', EOS_token:'EOS'}
    nrows = 0
    first = True
    ...
    with open(nw_file, 'r', newline='') as csvfile:
        ...
        for tup in freader:
            ...
            if freq >= min_freq:
                ...
                if first and freq == (min_freq - 1):
                    nrows = num
                    first = False
                w2idx[word] = UNK_token
    return w2idx, idx2w, nrows

class Voc:
    def __init__(self, nw_file=''):
        self.nw_file = nw_file
    
    def load(self):
        w2idx, idx2w, nrows = read_w2idx_idx2w(nw_file=self.nw_file)
        self.word2index = w2idx
        self.index2word = idx2w
        self.words = nrows - 1

voc = Voc(nw_file=NW_FILE)
voc.load()
print('load voc done.')

def get_corpus_pairs(data_file='', ignore_unk=True, words=0, step=2, maxlen=499):
    pairs = []
    max_original = 0
    max_summary = 0
    with open(data_file, 'r', newline='') as cfile:
        ...
        num_lines = len(lines)
        num = 0   
        for lx in range(0, num_lines, step):
            arr1 = list(map(int, lines[lx][:-1].split(' ')))[:maxlen]
            arr2 = list(map(int, lines[lx+1][:-1].split(' ')))[:maxlen]
            #arr = list(map(int, l[:-1].split(' ')))[:500]
            original = arr2
            summary = arr1
            if ignore_unk:
                sum2 = [i for i in summary if i > words]
                ori2 = [i for i in original if i > words]
                if len(sum2) ==  0 and len(ori2) == 0:
                    pairs.append([original, summary])
                    num = lx

            if num % 10000 == 0:
                print('loading %d...' % (num))
    print('%d pairs are retrieved.' % len(pairs))
    return pairs, max_original, max_summary

if IS_PREDICT is False:
    pairs, max_original, max_summary = get_corpus_pairs(data_file=DATA_FILE, ignore_unk=True, words=voc.words)
else:
    pairs, max_original, max_summary = [], 500, 500
######################################################################
...
def indexesFromSentence(voc, sentence, passthru=True):
    if passthru:
        return sentence + [EOS_token]
    else:
...
def batch2TrainData(voc, pair_batch):
    ...
    # print('lengths=%s, max_target_len=%d' % (lengths, max_target_len))
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer, embedding, encoder_n_layers, decoder_n_layers, save_dir, n_iteration, batch_size, print_every, save_every, clip, corpus_name, loadFilename):
    ...
    print("long training batch = %d" % len(training_batches))
    training_batches = [batch2TrainData(voc, [pairs[(i*batch_size + j)%(int(len(pairs)/batch_size))] for j in range(batch_size)]) for i in range(int(len(pairs) / batch_size)*20)]
    print("small training batch = %d" % len(training_batches))
    # Initializations
    print('Initializing ...')
    start_iteration = 1
    print_loss = 0
...
def evaluate(encoder, decoder, searcher, voc, sentence, max_length=500):
    ### Format input sentence as a batch
    # words -> indexes
    print('原文: ' + ''.join(sentence.split(' ')))
    new_sentence = sentence # taggingFast(sentence, retWithList=False)
    indexes_batch = [indexesFromSentence(voc, new_sentence, passthru=False)]
    # Create lengths tensor
    lengths = torch.tensor([len(indexes) for indexes in indexes_batch])
    # Transpose dimensions of batch to match models' expectations
    input_batch = torch.LongTensor(indexes_batch).transpose(0, 1)
    # print(new_sentence, input_batch, lengths)
    ...
    # indexes -> words
    decoded_words = [voc.index2word.get(token.item(), '[?]') for token in tokens]
    # print(decoded_words)
    return decoded_words


def evaluateJaText8(encoder, decoder, searcher, voc, filename='ja.text8.20180601.100MB'):
...
corpus_name = "cornell movie-dialogs corpus" # dummy
attn_model = 'dot'
#attn_model = 'general'
#attn_model = 'concat'
hidden_size = 128
...
if loadFilename is not None:
    ...
    else: # If loading a model trained on GPU to CPU
...
embedding = nn.Embedding(voc.words, hidden_size)
if loadFilename:
    embedding.load_state_dict(embedding_sd)
# Initialize encoder & decoder models
encoder = EncoderRNN(hidden_size, embedding, encoder_n_layers, dropout)
decoder = LuongAttnDecoderRNN(attn_model, embedding, hidden_size, voc.words, decoder_n_layers, dropout)
if loadFilename:
...
n_iteration = int(len(pairs) / batch_size) * 20 # 4000
print_every = 1
save_every = 500
...
evaluateJaText8(encoder, decoder, searcher, voc)
