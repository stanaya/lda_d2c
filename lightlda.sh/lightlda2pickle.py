...
def C_wk_w(dir, topics, beta0):
    data = svmlight.loadex (dir + '/' + 'server_0_table_0.model')
    nlex = nlexicon (data)
    matrix = np.zeros ((nlex,topics)) + beta0
    cw = np.zeros(nlex) + beta0
    vocabsum = 0
    for w, toc in data:
        L = len (toc.id)
        for j in range(L):
            ...
            cw[w] += c
            vocabsum += c
    return (matrix, cw, vocabsum, nlex)

def C_k(dir, topics, alpha0):
    data = svmlight.loadex (dir + '/' + 'server_0_table_1.model')
    matrix = np.zeros (topics) + alpha0
    for _, toc in data:
        L = len (toc.id)
        for j in range(L):
            k = toc.id[j]
...
def P_wk(dir, beta0, topics, nlex, cwk, cw, ck, N):
    normal = {}
    maniac = {}
    for k in range(topics):
        for w in range(nlex):
            ...
            if Cwk == beta0:
                continue
            normal[(w,k)] = Pwk
            maniac[(w,k)] = Pwk / (cw[w] / N)
    return normal, maniac

def lightlda2pickle(dir=None):
    model = {}
    alpha0,beta0,topics,_ = read_param (dir + '/' + 'param')
    model['alpha'] = alpha0
    model['topics'] = topics
    cwk, model['C(w)'], N, nlex = C_wk_w(dir, topics, beta0)
    model['N'] = N
    model['nlex'] = nlex
    model['C(k)'] = C_k(dir, topics, alpha0)
    model['P(w|k)'], model['P(w|k)/P(w)'] = P_wk(dir, beta0, topics, nlex, cwk, model['C(w)'], model['C(k)'], N)
    return model

def usage ():
    print('usage: lightlda2pickle dir')
    sys.exit (0)
...
def main ():
    ...
    dir = sys.argv[1]
    print('reading model..')
    model = lightlda2pickle (dir)
    print('saving model..')
    with gzip.open (dir + '/' + 'pickle4.gz', 'wb') as gf:
